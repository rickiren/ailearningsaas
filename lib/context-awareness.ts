...
export interface CodePatterns {
  namingConventions: {
    components: string; // 'PascalCase' | 'kebab-case' | 'snake_case'
    files: string; // 'PascalCase' | 'kebab-case' | 'snake_case'
    functions: string; // 'camelCase' | 'snake_case'
    constants: string; // 'UPPER_SNAKE_CASE' | 'camelCase'
  };
  fileExtensions: {
    components: string[]; // ['.tsx', '.jsx', '.vue']
    pages: string[]; // ['.tsx', '.jsx', '.vue']
    utilities: string[]; // ['.ts', '.js']
    styles: string[]; // ['.css', '.scss', '.module.css']
  };
  importPatterns: string[]; // ['import { Component } from "./Component"', ...]
  exportPatterns: string[]; // ['export default Component', 'export { Component }', ...]
  folderStructure: {
    components: string; // 'components/' | 'src/components/'
    pages: string; // 'pages/' | 'app/'
    utilities: string; // 'lib/' | 'utils/'
...
class ContextAwareness {
   ...
   */
  async detectCodePatterns(): Promise<CodePatterns> {
    ...
    try {
      ...
      for (const file of allFiles) {
        ...
        try {
          ...
          if (criteria === 'content') {
            // Simple content similarity (can be enhanced with more sophisticated algorithms)
            ...
            // Analyze purpose based on file location and content
  ...
  /**
   * Validate file path based on actual project structure
   */
  validatePath(filePath: string): { isValid: boolean; suggestions: string[]; reason?: string } {
    try {
      const fullPath = path.join(this.projectRoot, filePath);
      ...
      if (!fs.existsSync(dirPath)) {
        // Suggest creating the directory or using existing structure
        const suggestions = this.suggestValidPaths(filePath);
        return { 
          isValid: false, 
          suggestions, 
  ...
  async getContextAwareSuggestions(fileType: 'component' | 'page' | 'utility', name: string): Promise<{
    ...
    // Determine suggested path based on project structure
  ...
  private async analyzeComponent(filePath: string): Promise<ComponentInfo | null> {
    try {
      ...
      // Basic component analysis (can be enhanced with AST parsing)
  ...
  private isComponentFile(filePath: string): boolean {
    ...
    // Check if it's likely a component based on name and location
    return (
  ...
  private isPageFile(filePath: string): boolean {
    ...
    // Check if it's likely a page based on name and location
    return (
      (ext === '.tsx' || ext === '.jsx' || ext === '.vue') &&
  ...
  private isUtilityFile(filePath: string): boolean {
    ...
    // Check if it's likely a utility based on name and location
    return (
      (ext === '.ts' || ext === '.js') &&
  ...
  private async analyzeNamingConventions(): Promise<CodePatterns['namingConventions']> {
    // Analyze existing files to determine naming conventions
    return {
  ...
  private suggestValidPaths(invalidPath: string): string[] {
    const suggestions: string[] = [];
    const dir = path.dirname(invalidPath);
    
    // Try to find similar existing directories
    try {
      const existingDirs = fs.readdirSync(this.projectRoot, { withFileTypes: true })
        .filter(dirent => dirent.isDirectory())
        .map(dirent => dirent.name);
      
      for (const existingDir of existingDirs) {
        if (existingDir.toLowerCase().includes(path.basename(dir).toLowerCase())) {
          suggestions.push(path.join(existingDir, path.basename(invalidPath)));
        }
      }
    } catch (error) {
      // Ignore errors
...
export const contextAwareness = new ContextAwareness(process.cwd());
