...
import { Bot, User, FileText, CheckCircle, Copy, Check, AlertTriangle } from 'lucide-react';
...
import { StreamingText, useStreamingText } from './streaming-text';
import { 
...
export function StreamingChatMessage({ 
  ...
  const displayText = isCurrentlyStreaming ? streamingText : message.content;
  
  // Process AI message to separate conversational text from JSON
  const { displayContent, jsonData } = !isUser && displayText
    ? processAIMessage(displayText) 
    : { displayContent: displayText, jsonData: null };
          ...
          <div className="flex-1 min-w-0 space-y-3">
            ...
            <SlideInFromBottom delay={200}>
              <div className={cn(
                ...
                <div className={cn(
                  'text-sm leading-relaxed whitespace-pre-wrap break-words min-h-[20px]',
                  ...
                  {isUser ? (
                    ...
                    // AI messages stream in real-time
                    <>
                      {displayContent ? (
                        <StreamingText 
                          text={displayContent}
                          isComplete={!isCurrentlyStreaming}
                          speed={25}
                          onComplete={() => {
                            console.log('Streaming completed for message:', message.id);
                          }}
                        />
                      ) : isCurrentlyStreaming ? (
                ...
                {!isUser && displayContent && !isCurrentlyStreaming && (
                  <HoverCard>
                    <button
                      onClick={handleCopy}
                      className={cn(
            ...
            <SmoothHeight>
              {hasMindmap && mindmapData && !isCurrentlyStreaming && (
